...
# ==================================================================================
# [THESIS DEFINITION] 
# The Theoretical Action Space mapping. 
# ==================================================================================
ACTION_TO_WEIGHTS_FIXED = {
    # --- Validated Actions (Active in Q-Net) ---
    0: {'short': 0.4875, 'long': 0.2625, 'physics': 0.25}, # Maintain (Baseline)
    1: {'short': 0.5500, 'long': 0.2500, 'physics': 0.20}, # Fine-Tune Short (UNLOCKED)
    2: {'short': 0.3000, 'long': 0.4500, 'physics': 0.25}, # Fine-Tune Long
    3: {'short': 0.3250, 'long': 0.1750, 'physics': 0.50}, # Recalibrate Physics

    # --- Pruned Actions (Fallback) ---
...
def _infer_state_cols(df: pd.DataFrame, qnet_in_dim: int) -> List[str]:
    ...
    if not preferred:
        exclude = {"action", "a", "reward", "r", "done", "terminal", "forecast_start", "blend_short", "blend_long", "blend_physics"}
        numeric = [c for c in cols if c not in exclude and not c.startswith("next_") and pd.api.types.is_numeric_dtype(df[c])]
        preferred = numeric
...
def _extract_qnet_state_dict(ckpt: Dict[str, Any]) -> Dict[str, torch.Tensor]:
    ...
    if all(isinstance(v, torch.Tensor) for v in ckpt.values()): return ckpt
    raise ValueError(f"Could not find q-network state_dict keys.")

def _build_qnet_from_state_dict(sd: Dict[str, torch.Tensor]) -> Tuple[QNet, int, int]:
    weight_keys = sorted([k for k in sd.keys() if k.endswith(".weight") and k.startswith("net.")], key=lambda k: int(k.split(".")[1]))
    layer_sizes = []
...
def main() -> None:
    ...
    forecaster = PhysicsAwareForecaster(
        short_ckpt=str(args.short_ckpt), long_ckpt=str(args.long_ckpt),
        plant_metadata=str(args.plant_meta), short_train_parquet=str(args.short_train),
        long_train_parquet=str(args.long_train), device=str(device)
    )
    ...
    writer = None
    stats = {"attempted": 0, "saved": 0, "fallback_no_state": 0, "skipped_bad_window": 0, "skipped_failed": 0}
    ...
    for fs in fss:
        ...
        try:
            ...
            hwin = hist_df[(hist_df["timestamp_utc"] < fs) & (hist_df["timestamp_utc"] >= hstart)].copy() if hist_df is not None else wwin.iloc[:0]
            if len(hwin) == 0: hwin = wx15[wx15["timestamp_utc"] < fs].tail(96).copy()
            ...
            # --------------------------------------------------------
            # CRITICAL INDENTATION FIX:
            # This block is now strictly OUTSIDE the if/else above
            # and strictly INSIDE the try/except block.
            # --------------------------------------------------------
            ...
            yhat = forecaster.predict_30d(fs, wwin, hwin, blend_weights, return_components=False)
            yhat = np.asarray(yhat, dtype=np.float32).reshape(-1)
            
            ts = pd.date_range(start=fs, periods=2880, freq="15min", tz="UTC")
            out_df = pd.DataFrame({
                "timestamp_utc": ts, "forecast_start": fs, "step_ahead": np.arange(2880),
                "predicted_power_norm": yhat, "policy_action": a,
                "blend_short": blend_weights["short"], "blend_long": blend_weights["long"], "blend_physics": blend_weights["physics"]
            })
            
            table = pa.Table.from_pandas(out_df, preserve_index=False)
            if writer is None: writer = pq.ParquetWriter(out_path, table.schema, compression="snappy")
            writer.write_table(table)
            stats["saved"] += 1
        except Exception as e:
            LOGGER.error(f"Failed fs={fs}: {e}")
