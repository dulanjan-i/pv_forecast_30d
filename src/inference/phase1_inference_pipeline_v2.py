...
LOGGER = logging.getLogger("phase1_inference_pipeline_v2")
...
def _tft_only_from_components(components: Dict[str, Any]) -> np.ndarray:
    """
    Build TFT-only dual-head 15-min forecast from:
      - short_head_daily: (30,96)
      - long_upsampled: (2880,)
...
def main() -> None:
    ...
    ap.add_argument("--weather-source", choices=["historical", "api"], default="historical")
    ...
    ap.add_argument("--pred-mode", choices=["hybrid", "pvlib_only", "tft_only"], default="hybrid")
    ...
    if args.weather_source != "historical":
        raise NotImplementedError("Only historical mode is supported in this v2 script.")
    ...
    LOGGER.info("date_range=%s..%s stride_days=%d", start_utc, end_utc, stride_days)
    LOGGER.info("weather_max_ts=%s latest_valid_start=%s run_end=%s", max_ts, latest_start, run_end)
    LOGGER.info("sharding shard_idx=%d num_shards=%d starts=%d", args.shard_idx, args.num_shards, len(starts))
    ...
    for k, fs in enumerate(starts, start=1):
        ...
        if need_components:
            ...
            short_daily = np.asarray(comp["short_head_daily"], dtype=np.float32)
            pred_short_15 = short_daily.reshape(-1)
            pred_long_15 = np.asarray(comp["long_upsampled"], dtype=np.float32).reshape(-1)
        ...
        for i in range(2880):
            ...
            if save_components:
                ...
                r["pred_short_norm"] = float(pred_short_15[i])
                r["pred_long_norm"] = float(pred_long_15[i])
                r["pred_hybrid_norm"] = float(pred_hybrid[i])
            rows.append(r)

        if k % 25 == 0:
            LOGGER.info("progress %d/%d in this shard", k, len(starts))
    ...
    LOGGER.info("starts attempted=%d skipped_incomplete=%d rows=%d", len(starts), skipped_incomplete, len(out_df))
