...
def plot_facets_case_week(
    ...
    for i, name in enumerate(names):
        ...
        ax.plot(tt["timestamp_utc"].values, tt["y_true"].values, label="truth")
        ax.plot(bt["timestamp_utc"].values, bt["y_pred"].values, label="baseline")
...
def plot_facets_abs_error_hist(
    ...
    for i, name in enumerate(names):
        ...
        ax.hist(b_abs, bins=80, alpha=0.5, label="baseline")
        ax.hist(m_abs, bins=80, alpha=0.5, label=name)
        ax.set_title(name)
...
def plot_facets_leadtime_rmse_curve(
    ...
    for i, name in enumerate(names):
        ...
        ax.plot(b_curve["hours_ahead"].values, b_curve["RMSE"].values, label="baseline")
...
def main() -> None:
    ...
    plot_facets_case_week(
        ...
        title=f"Case study summer week, truth vs {args.baseline_name} vs each model",
    )
    ...
    plot_facets_abs_error_hist(
        ...
        title=f"Absolute error histogram, baseline vs each model",
        max_abs=1.0,
    )
    ...
    plot_facets_leadtime_rmse_curve(
        ...
        title=f"Lead-time RMSE curve (0–{args.max_hours_curve}h), baseline vs each model",
    ...
    print(f"[OK] Wrote benchmark outputs to: {paths.out_dir}")
