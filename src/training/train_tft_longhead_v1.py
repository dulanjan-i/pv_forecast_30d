...
Key properties (same as v1):
- Manual PyTorch training loop (NO Lightning Trainer)
- Safe time_idx rebuilt via groupby cumcount()
- allow_missing_timesteps=True (critical for hourly)
- Saves best.ckpt / last.ckpt as pure state_dict (same convention as v1)

Defaults are longhead-friendly:
- pred_len=720 (30 days hourly)
- encoder_len=168 (7 days history)  <-- override if you want
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="Optional torch-saved model.state_dict() to warm-start from.",
    )
    p.add_argument(
        "--init_strict",
        action="store_true",
...
def _progress_line(prefix: str, epoch: int, step: int, total: int, t0: float) -> None:
    ...
    print(f"[{prefix} e{epoch}] {step}/{total} ({pct:.1f}%) it/s={it_s:.2f} ETA={remaining/60:.1f} min", flush=True)
...
def main() -> None:
    ...
    print("=" * 80, flush=True)
    print("Longhead TFT training (v1-compatible)", flush=True)
    print(f"train_parquet: {args.train_parquet}", flush=True)
    print(f"val_parquet:   {args.val_parquet}", flush=True)
    print(f"run_dir:       {run_dir}", flush=True)
    print(f"encoder_len:   {args.encoder_len}", flush=True)
    print(f"pred_len:      {args.pred_len}", flush=True)
    print("=" * 80, flush=True)
    ...
    if args.init_state_dict:
        ...
        print(f"[INFO] Warm-start loaded from {init_path} strict={strict}", flush=True)
        if missing:
            print(f"[WARN] Missing keys: {missing[:20]}{' ...' if len(missing) > 20 else ''}", flush=True)
        if unexpected:
            print(f"[WARN] Unexpected keys: {unexpected[:20]}{' ...' if len(unexpected) > 20 else ''}", flush=True)
    ...
    for epoch in range(int(args.max_epochs)):
        ...
        if int(args.patience) > 0 and bad_epochs >= int(args.patience):
            print(f"[INFO] Early stop. No improvement for {bad_epochs} epochs. Best epoch={best_epoch} val={best_val_loss:.4f}", flush=True)
            break
